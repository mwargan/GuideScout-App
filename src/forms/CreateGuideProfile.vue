...
const handleSubmit = async () => {
  ...
  try {
    await userStore.register(finalData);
  } catch (error) {
    ...
    console.error("Error submitting form data", inputErrors);
...
const baseFormRef = ref<typeof BaseForm>();
</script>
...
<template>
  ...
  >
    ...
    <base-button
      ...
      :disabled="isLoadingFile"
      @click="handleSubmit"
      >{{ isLastStep ? $t("Sign up") : $t("Next") }}</base-button
    >
